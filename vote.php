...
else
{
    ...
    else
    {
        ...
        if($thisSession->questionMode == 0)
        {
	        ...
	        {
                $template->pageData['mainBody'] .= displayQuestion($thisSession->currentQuestion, $cidx);
	        }
        ...
        else
        {
	        if((isset($thisSession->extras[currentQuestions]))&&(is_array($thisSession->extras[currentQuestions]))&&(sizeof($thisSession->extras[currentQuestions]) >= 0))
            {
                ...
                $template->pageData['mainBody'] .= displayQuestion($cqiid, true, $cidx);
	            ...
	            if($cidx < sizeof($thisSession->extras[currentQuestions])-1)
	            {
		            ...
		            $template->pageData['mainBody'] .= "<a href='vote.php?sessionID={$thisSession->id}&qiID={$next}' id='next' style='display: none;' class='pull-right'>Continue to Next Question &rsaquo;</a>";
	            }
...
function displayQuestion($qiid, $forceTitle=false, $cidx)
{
     ...
     if($qu)
     {
        ...
        //$out .= '<pre>'.print_r($resp,1).'</pre>';
         ...
         {
             $out .= "<input type='submit' id='save' style='display: none;' name='submitans' value='Save Answer' class='btn btn-primary' onclick=checkconfidence(); />";
         }
         ...
         $out .= "</div>";
         $out .="<div class='col-md-5 pull-right' style='background: #F1EEEE; padding: 10px;'>";
         $out .="<b><small>How confident are you with your answer?</small></b>";
         $out .="<br><label style='cursor: pointer;'><input type='radio' name='cq' value='1' onclick=checkconfidence(); /> Very Confident</label>";
         $out .="<br><label style='cursor: pointer;'><input type='radio' name='cq' value='2' onclick=checkconfidence(); /> Confident</label>";
         $out .="<br><label style='cursor: pointer;'><input type='radio' name='cq' value='3' onclick=checkconfidence(); /> Partially Confident</label>";
         $out .="<br><label style='cursor: pointer;'><input type='radio' name='cq' value='4' onclick=checkconfidence(); /> All Guess</label>";
         $out .="</div>";
